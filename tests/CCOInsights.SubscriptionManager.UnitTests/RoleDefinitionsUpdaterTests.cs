...
public class RoleDefinitionsUpdaterTests
{
    ...
    public async Task UpdateAsync_ShouldUpdate_IfValid()
    {
        ...
        _storageMock.Verify(x => x.UpdateItemAsync(It.IsAny<string>(), It.IsAny<string>(), It.Is<List<RoleDefinitions>>(x => x.Any(item => item.SubscriptionId == subscriptionTest.SubscriptionId && item.TenantId == subscriptionTest.Inner.TenantId)), It.IsAny<CancellationToken>()), Times.Once);
    }
}
