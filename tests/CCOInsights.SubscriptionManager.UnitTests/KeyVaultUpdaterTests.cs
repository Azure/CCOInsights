...
namespace CCOInsights.SubscriptionManager.UnitTests
{
    public class KeyVaultUpdaterTests
    {
        ...
        public async Task KeyVaultUpdater_UpdateAsync_ShouldUpdateNics_IfValid()
        {
            ...
            _storageMock.Verify(x => x.UpdateItemAsync(It.IsAny<string>(), It.Is<KeyVault>(x => x.SubscriptionId == subscriptionTest.SubscriptionId && x.TenantId == subscriptionTest.Inner.TenantId), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task KeyVaultUpdater_UpdateAsync_ShouldNotUpdateNics_IfNotValid()
        {
            ...
            _providerMock.Verify(x => x.GetAsync(It.Is<string>(x => x == subscriptionTest.SubscriptionId), CancellationToken.None));
            _storageMock.Verify(x => x.UpdateItemAsync(It.IsAny<string>(), It.Is<Nic>(x => x.SubscriptionId == subscriptionTest.SubscriptionId && x.TenantId == subscriptionTest.Inner.TenantId), It.IsAny<CancellationToken>()), Times.Never);
        }
    }
}
