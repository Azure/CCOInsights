...
public class VirtualNetworksUpdaterTests
{
    ...
    public async Task UpdateAsync_ShouldUpdate_IfValid()
    {
        ...
        _storageMock.Verify(x => x.UpdateItemAsync(It.IsAny<string>(), It.IsAny<string>(), It.Is<List<VirtualNetworks>>(x => x.Any(item => item.SubscriptionId == subscriptionTest.SubscriptionId && item.TenantId == subscriptionTest.Inner.TenantId)), It.IsAny<CancellationToken>()), Times.Once);
    }
}
