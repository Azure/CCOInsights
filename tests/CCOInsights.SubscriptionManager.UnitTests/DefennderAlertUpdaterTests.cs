...
public class DefenderAlertUpdaterTests
{
    ...
    public async Task DefenderAlertUpdater_UpdateAsync_ShouldUpdate_IfValid()
    {
        ...
        _storageMock.Verify(x => x.UpdateItemAsync(It.IsAny<string>(), It.IsAny<string>(), It.Is<DefenderAlert>(x => x.SubscriptionId == subscriptionTest.SubscriptionId && x.TenantId == subscriptionTest.Inner.TenantId), It.IsAny<CancellationToken>()), Times.Once);
    }
}
